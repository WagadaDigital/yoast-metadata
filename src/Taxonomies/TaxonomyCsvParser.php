...
/**
 * Parses and validates CSV files for taxonomy term import.
 */
final class TaxonomyCsvParser {

	/**
	 * Required CSV headers.
	 */
	private const REQUIRED_HEADERS = [ 'slug', 'taxonomy' ];

	/**
	 * Optional CSV headers.
	 */
	private const OPTIONAL_HEADERS = [ 'title', 'description', 'focuskw', 'canonical', 'name', 'url', 'noindex', 'nofollow' ];
	...
	private function parse_row( array $row ): ?array {
		...
		// Skip rows without slug or taxonomy.
		if ( empty( $parsed['slug'] ) || empty( $parsed['taxonomy'] ) ) {
			return null;
		}
