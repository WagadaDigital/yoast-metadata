...
/**
 * Handles AJAX import requests for taxonomy terms.
 */
final class TaxonomyImportHandler {

	private TermMetaHandler $meta_handler;
	private ?TaxonomyCsvParser $parser = null;
	private ?TaxonomyBatchProcessor $processor = null;
	...
	public function register(): void {
		add_action( 'wp_ajax_' . Plugin::PREFIX . 'upload_taxonomy_csv', [ $this, 'handle_upload' ] );
		add_action( 'wp_ajax_' . Plugin::PREFIX . 'start_taxonomy_import', [ $this, 'handle_start_import' ] );
		add_action( 'wp_ajax_' . Plugin::PREFIX . 'process_taxonomy_batch', [ $this, 'handle_process_batch' ] );
	}
	 ...
	 */
	private function get_parser(): TaxonomyCsvParser {
		if ( null === $this->parser ) {
			$this->parser = new TaxonomyCsvParser();
		}
		return $this->parser;
	}
	 ...
	 */
	private function get_processor(): TaxonomyBatchProcessor {
		if ( null === $this->processor ) {
			$this->processor = new TaxonomyBatchProcessor( $this->meta_handler );
		}
